    ...
    HTTPClient('post', 'console', DELETE_RESOURCE(args.argv[2], args.argv[3], args.argv[4], args.argv[5], args.argv[6]))
      .then(res => {
        ...
        if (data.deleteResource && !deletedResources.includes(args.argv[2])) {
          ...
          resolve(res.errors ? res.errors[0] : strings('modal.deleted.resource', [args.argv[2]]))
        } else {
...
export const searchDelete = args =>
  new Promise(resolve => {
    switch (args.argv[1]) {
      case '-delete="save"':
