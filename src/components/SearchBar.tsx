...
export default class SearchBar extends React.Component<SearchBarProps, SearchBarState> {
  ...
  componentWillReceiveProps(nextProps) {
    if (!_.isEqual(nextProps.availableFilters, this.state.fieldOptions)) {
      const fields =
        nextProps.availableFilters && _.get(nextProps, 'availableFilters[0].id', '') !== 'failed'
    ...
    if (nextProps.value === '') {
      ...
      const currentOperator =
        this.state.operators[this.state.operators.findIndex(op => op === tags[tags.length - 1].matchText[0])] || null
      const field =
        (tags[tags.length - 1].classType === 'keyword' || _.get(tags[tags.length - 1], 'matchText[0]', '') !== '') &&
  ...
  formatSuggestionOptions(data) {
    ...
    if (searchComplete !== '' && data && data.searchComplete) {
      ...
      if (tags.length > 1) {
        ...
        if (kindTag.length > 0) {
          data.searchComplete = data.searchComplete.filter(
            value => kindTag[0].matchText.findIndex(item => item === value) === -1
          )
        }
      }
  ...
  handleDelete(i) {
    ...
    if (tags.length > 0) {
      if (
        tags[i]['matchText'] === undefined || // If tag only contains a filter value
        (tags[i]['matchText'] && tags[i]['matchText'].length <= 1) || // If tag contains a filter and only 1 filter type ex: (kind:pod)
        (tags[i]['classType'] === 'keyword' &&
          ((tags.length > 1 && tags[i]['value'] !== 'search') || (tags.length === 1 && tags[i]['value'] === 'search')))
      ) {
        // dont allow deletion of search tag if there are other tags being used
  ...
  updateSelectedTags(tags, currentTag) {
    ...
    if (
      ...
      matchText &&
      matchText.length > 0 &&
      this.state.operators.findIndex(op => op === matchText[0]) === -1
    ) {
      this.setState({
        currentTag: {
          field: '',
  ...
  render() {
    ...
    if (searchComplete !== '' && suggestions.length === 0) {
      ...
      HTTPClient('post', 'search', GET_SEARCH_COMPLETE(searchComplete, query)).then(res => {
      ...
      <div className="tagInput-filter">
        <div className={'tagInput-comboBox'} onKeyPress={this.props.onKeyPress}>
          <ReactTags
            ...
            allowNew={true}
            delimiterChars={
              _.get(fieldOptions, '[0].id', '') === 'loading' || fieldOptions.length > 1 ? [' ', ':', ','] : []
            }
            delimiters={[9]}
            autofocus={true}
