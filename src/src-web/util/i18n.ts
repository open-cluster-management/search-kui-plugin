...
export default (key: string, args?: Array<any>) => {
  const defaultStrings: Record<string, string> = require(`@kui-shell/plugin-search/i18n/en-US.json`)
  ...
  const i18n = (locale: string): Record<string, string> => {
      ...
      try {
          return (
          (locale && require(`@kui-shell/plugin-search/i18n/${locale.replace(/-.*$/, '')}.json`)) || defaultStrings
          )
      } catch (err) {
